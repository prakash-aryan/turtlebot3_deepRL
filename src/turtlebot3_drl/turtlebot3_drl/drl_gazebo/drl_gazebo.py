...
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.entity_dir_path = os.path.join(
            drl_gz_share, 'models', 'turtlebot3_drl_world', 'goal_box')
        ...
        self.world_name = f'drl_stage{self.stage}'
        ...
        ************************************************************"""
        ...
        self.control_world_client = self.create_client(
            ControlWorld, f'/world/{self.world_name}/control')
    ...
    *******************************************************************************"""
    ...
    def publish_callback(self):
        ...
        goal_pose.position.y = self.goal_y
        self.goal_pose_pub.publish(goal_pose)
        if getattr(self, 'goal_spawned', False):
    ...
    def move_goal_entity(self):
        ...
        p = Pose()
        p.position.x = self.goal_x
        p.position.y = self.goal_y
        p.position.z = 0.0
        p.orientation.w = 1.0
        req.pose = p
    ...
    def reset_simulation(self):
        ...
        pose_req.entity = Entity(name=self.robot_name, type=Entity.MODEL)
        pose = Pose()
        pose.position.x = 0.0
        pose.position.y = 0.0
        ...
        while not self.set_pose_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('set_pose service not available, waiting again...')
        self.set_pose_client.call_async(pose_req)
    ...
    def spawn_entity(self):
        ...
        factory.name = self.entity_name
        factory.allow_renaming = False
        factory.sdf = self.entity
        factory.pose = goal_pose
    ...
    def get_obstacle_coordinates(self):
        ...
        inner_walls_sdf = os.path.join(
            drl_gz_share, 'models', 'turtlebot3_drl_world', 'inner_walls', 'model.sdf')
